...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angle = Math.acos(dotProd / (mag1 * mag2));
  return angle;
}
...
function getSumOfDigits(num) {
  return `${num}`.split('').reduce((acc, cur) => +acc + +cur);
}
...
function toPrecision(number, precision) {
  const roundedNumber = number.toPrecision(precision);
...
function getSumOfNumbers(x1, x2, x3) {
  return Math.floor((x1 + x2 + x3) * 10) / 10;
}
...
function getRandomInteger(min, max) {
  const funcMin = min - 0.5;
  const funcMax = max + 0.5;

  const number = funcMin + Math.random() * (funcMax - funcMin);
  return Math.round(number);
}
